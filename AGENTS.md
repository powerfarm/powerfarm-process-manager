...
- **Never commit secrets.** `.env*` is gitignored. Notion and Resend authorize per user via Vercel Connect, with their connector UIDs read from `NOTION_CONNECTOR` and `RESEND_CONNECTOR`. GitHub uses the private PowerFarm GitHub App; its installation is restricted to `POWERFARM_PLANNING_REPOSITORY`, and its ID, installation ID, and private key live only in the environment. Blob auth is via the project's OIDC token. `TYPEFULLY_API_KEY` is read from the environment inside `getToken`, never inlined. The Resend connector only issues `user` tokens, so there is no app-scoped fallback: a session with no resolved principal fails with `principal_required` rather than quietly sending as the app.
