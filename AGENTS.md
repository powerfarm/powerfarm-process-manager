...
- **The composer and the channel share one policy.** `lib/chat/attachments.ts` holds the size cap, the media-type allow list, and how many files one message may carry; `agent/channels/eve.ts` passes the same values to `uploadPolicy`. Change the limits there, never in one of the two call sites: the browser copy exists to fail early with a filename, and the channel is the authority that answers 413 or 415.
- **Attachments travel as bytes, not as links.** A `data:` URL is what eve stages into the session sandbox under `/workspace/attachments`, which is the whole point: the lead opens a real file with `bash`. A remote URL would pass straight through to the model provider unstaged. The cap follows from the platform's request-body limit rather than from anything eve enforces.
- **Inline bytes never reach storage.** `withoutInlineAttachmentBytes` drops the `data:` URL that `message.received` echoes back, keeping filename, media type, and size. Every event in a chat is persisted, in the browser or in Neon, so the copy would otherwise live as long as the chat does.
- **`share_file` is the return path, and it reads the sandbox itself.** That is the difference from `upload_asset`, which takes content as tool input and therefore moves the whole document through the model's context. Shared files land under `shared/`, which is deliberately not a reserved prefix: a briefed specialist has to be able to `download_asset` them.
...
- The lead alone receives the eleven process tools. Specialists receive a bounded title/projection/graph excerpt in their brief and return work; they never mutate the process directly. `observe_process_artifact` is the only tool that may write `governed_artifact` provenance; the generic graph mutation tool must reject that kind and its reserved metadata.
...
- The pill is bound to one conversation, because it is reconstructed from that chat's own `action.result` events. The sidebar list is the way into a process from anywhere else, and it reads the same owner-scoped routes. Neither can activate a process: binding a session is `activate_process`, a lead tool, which is why the list opens a panel rather than a chat.
...
- **Never commit secrets.** `.env*` is gitignored. Notion and Resend authorize per user via Vercel Connect, with their connector UIDs read from `NOTION_CONNECTOR` and `RESEND_CONNECTOR`. GitHub uses the private PowerFarm GitHub App; its organization installation may be broad, but every lead token is restricted to read-only access on `POWERFARM_PLANNING_REPOSITORY`. The App ID, installation ID, and private key live only in the environment. Any future coding surface must mint a separate bounded write token and must not expose merge. Blob auth is via the project's OIDC token. `TYPEFULLY_API_KEY` is read from the environment inside `getToken`, never inlined. The Resend connector only issues `user` tokens, so there is no app-scoped fallback: a session with no resolved principal fails with `principal_required` rather than quietly sending as the app.
