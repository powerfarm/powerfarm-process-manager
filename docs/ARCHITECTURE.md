...
| GitHub reading | `agent/extensions/github.ts`, `agent/lib/github/` | extension + tool dependency | Mounts an exact read-only GitHub tool list over the repository-scoped PowerFarm GitHub App. The custom artifact observer resolves a requested ref, hashes the file bytes, reads PR/review/check state, and hands those facts to the Process service as one trusted observation. |
...
| GitHub | governed planning files and their immutable repository state | `@github-tools/eve-extension` plus short-lived installation tokens from the repository-scoped PowerFarm GitHub App |
...
- **Connectors:** Notion (`NOTION_CONNECTOR`, defaulting to `notion/marketing-team`) and Resend (`RESEND_CONNECTOR`, defaulting to `resend/marketing-team`) are user-scoped integrations. Slack (`SLACK_CONNECTOR`) is optional and needs triggers pointed at `/eve/v1/slack`. GitHub does not use Vercel Connect: the private PowerFarm GitHub App is installed only on `POWERFARM_PLANNING_REPOSITORY`, and Octokit mints cached, short-lived installation tokens directly. Blob is provisioned as a store, not a connector.
