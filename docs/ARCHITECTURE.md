...
| Process tools | `agent/tools/*process*.ts`, `agent/lib/processes/tools.ts` | tools | Eleven lead-only tools create, bind, query, and commit typed graph, state, tag, and governed artifact operations. Server context supplies owner, session, turn, tool, and `ctx.callId`; GitHub supplies observed revisions and governance facts; the model supplies none of that provenance. |
| GitHub reading | `agent/extensions/github.ts`, `agent/lib/github/` | extension + tool dependency | Mounts an exact read-only GitHub tool list. Although the PowerFarm GitHub App installation spans the organization, this runtime mints a token narrowed to `POWERFARM_PLANNING_REPOSITORY` and read permissions. The custom artifact observer resolves a requested ref, hashes the file bytes, reads PR/review/check state, and hands those facts to the Process service as one trusted observation. |
| Process store | `lib/processes/`, `lib/db/schema.ts` | application Postgres | Stores process envelopes, nodes, edges, materialized human projections, and an append-only event log in the existing `DATABASE_URL`. Node and edge semantic ids are local to their process. Mutations use row locking, expected versions, and unique mutation keys in a transaction; generic mutations cannot fabricate governed artifact provenance. |
...
- **Postgres process store** — `process`, `process_node`, `process_edge`, `process_projection`, and append-only `process_event` tables in the existing `DATABASE_URL`. It is owner-scoped and crosses Eve sessions. Node and edge ids are composite with `process_id`, so the same semantic ids may recur safely in different processes. `process_projection` is a rebuildable read model; `process_event` rejects update and delete.
- **GitHub planning repository** — owns governed planning bytes, commits, diffs, pull requests, reviews, and checks. Process stores only its exact observation of a file: repository, path, requested locator, resolved commit, content digest, governance state, and server timestamp.
...
| GitHub | governed planning files and their immutable repository state | `@github-tools/eve-extension` plus a short-lived PowerFarm GitHub App token reduced to the configured planning repository and read permissions |
...
- **Connectors:** Notion (`NOTION_CONNECTOR`, defaulting to `notion/marketing-team`) and Resend (`RESEND_CONNECTOR`, defaulting to `resend/marketing-team`) are user-scoped integrations. Slack (`SLACK_CONNECTOR`) is optional and needs triggers pointed at `/eve/v1/slack`. GitHub does not use Vercel Connect: the private PowerFarm GitHub App has an organization-wide installation, while Octokit mints the lead a cached, short-lived token reduced to read-only access on `POWERFARM_PLANNING_REPOSITORY`. A future coding surface must mint a separate bounded write token and expose branch, commit, and draft-PR operations without merge. Blob is provisioned as a store, not a connector.
- **Environment:** starter mode adds `EVE_CHAT_PASSWORD` as its shared access credential. Adding a migrated `DATABASE_URL` enables the process store without changing password auth or browser chat storage. Full production mode combines Better Auth, Neon, and Upstash variables. GitHub App credentials and `TYPEFULLY_API_KEY` are static outbound-service credentials kept in Vercel environment variables. Notion and Resend are authorized per user, while Blob and the model use the project's OIDC token. Resend needs a verified sending domain before anything can be sent.
